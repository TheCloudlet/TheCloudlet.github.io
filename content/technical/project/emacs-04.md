...
## 1. A Hidden Field in Lisp_String
...
## 2. The Problem with Absolute Coordinates
...
> <span style="color: red;">Hello, </span><span style="color: green;">world</span><span style="color: blue; font-weight: bold;">!</span>
...
_(Please jump to [Section 7 (Appendix)](#7-appendix-debugging-emacs-with-lldb) if you need a guide on how to compile a debug build of Emacs.)_
...
```c,linenos,hl_lines=14 20-21 30-31
...
## 5. Weight-Balanced vs. Depth-Balanced
...
### 5.1 The Coordinate Problem: Searching by Node vs. Searching by Index
...
In a text editor, "text length" is the physical spatial coordinate. By ensuring the `total_length` of the left and right subtrees are roughly equal (a concept rooted in Nievergelt and Reingold's 1973 BB[α] trees [^1] and later popularized by the Rope data structure [^2]), Emacs guarantees that every time it steps left or right, it eliminates half of the remaining text length. This relative, length-based navigation achieves optimal $O(\log W)$ traversal speed (where $W$ is the total text weight), regardless of how wildly disproportionate the metadata nodes are.

### 5.2 Amortized Cost and Lazy Rebalancing
...
Emacs's weight-balanced tree, on the other hand, consciously relaxes. It accepts imbalance as a feature, not a bug. As Sleator and Tarjan demonstrated with Self-Adjusting Binary Search Trees [^3], amortized, lazy rebalancing can match the asymptotic performance of eager rebalancing by fundamentally reframing what 'balanced' actually means.
...
### 5.3 Interval Tree and Gap Buffer Synergy
...
[^1]: J. Nievergelt and E. M. Reingold (1973). "Binary Search Trees of Bounded Balance". _SIAM Journal on Computing_.
[^2]: Hans-J. Boehm, Russ Atkinson, and Michael Plass (1995). "Ropes: an Alternative to Strings". _Software: Practice and Experience_.
[^3]: Daniel D. Sleator and Robert E. Tarjan (1985). "Self-Adjusting Binary Search Trees". _Journal of the ACM_.
[^4]: Charles Crowley (1998). "Data Structures for Text Sequences". _University of New Mexico Technical Report_.
...
## 7. Appendix: Debugging Emacs with LLDB

### 7.1. Build Emacs from Source
