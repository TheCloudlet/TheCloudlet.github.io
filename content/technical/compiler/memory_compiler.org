...
#+DATE: 2026-05-29
#+ZOLA_TAXONOMIES_CATEGORIES: compiler-design
#+ZOLA_TAXONOMIES_TAGS: memory-compiler sram eda layout-vs-schematic tiling
#+ZOLA_EXTRA_MATH: true
#+ZOLA_EXTRA_MATH_AUTO_RENDER: true
...
The classical representation is the *[[https://en.wikipedia.org/wiki/Tombstone_diagram][T-diagram]]* (or Tombstone diagram). It characterizes a compiler by three languages: the source language it reads (A), the target language it emits (B), and the implementation language it is written in (C):
...
The fundamental building block of any [[https://en.wikipedia.org/wiki/Static_random-access_memory][SRAM (Static Random-Access Memory)]] is the *bitcell* — a tiny circuit that holds one bit. Hardware teams design and characterize these cells by hand; the memory compiler's job is to replicate them at scale.
...
| View       | Format               | Used by                                                |
|------------+----------------------+--------------------------------------------------------|
| Layout     | [[https://en.wikipedia.org/wiki/GDSII][GDS]] / [[https://en.wikipedia.org/wiki/Open_Artwork_System_Interchange_Standard][OASIS]]          | Tapeout, sent to fab                                   |
| Abstract   | [[https://en.wikipedia.org/wiki/Library_Exchange_Format][LEF]]                  | [[https://en.wikipedia.org/wiki/Place_and_route][P&R (Place and Route)]] — exposes pins + blockage only   |
| Timing     | ~.lib~ ([[https://en.wikipedia.org/wiki/Liberty_File_Format][Liberty]])     | [[https://en.wikipedia.org/wiki/Static_timing_analysis][STA (Static Timing Analysis)]] — setup/hold, access time |
| Behavioral | Verilog / SV         | RTL simulation                                         |
| Netlist    | SPICE / [[https://en.wikipedia.org/wiki/Circuit_description_language][CDL]]          | SPICE simulation, [[https://en.wikipedia.org/wiki/Layout_Versus_Schematic][LVS]]                                  |
| Power      | ~.lib~ / [[https://en.wikipedia.org/wiki/Common_Power_Format][CPF]] / [[https://en.wikipedia.org/wiki/Unified_Power_Format][UPF]]   | Power analysis                                         |
| Test       | [[https://en.wikipedia.org/wiki/Automatic_test_pattern_generation][ATPG]] / [[https://en.wikipedia.org/wiki/Built-in_self-test][MBIST]] pattern | [[https://en.wikipedia.org/wiki/Design_for_testing][DFT (Design for Testability)]] / manufacturing test      |
...
- Sign-off: [[https://en.wikipedia.org/wiki/Design_rule_check][DRC (Design Rule Check)]], [[https://en.wikipedia.org/wiki/Layout_Versus_Schematic][LVS (Layout vs Schematic)]], antenna checks, electromigration
...
L1 needs concurrent read and write because the pipeline demands it — hence 8T. But an 8T array is 1.5x–2x the area of 6T. At L3 scale (tens of megabytes), the area overhead of 8T becomes prohibitive, making it difficult to satisfy standard [[https://en.wikipedia.org/wiki/Power,_Performance,_Area][PPA (Power, Performance, Area)]] targets.

In SoC modeling, all of these decisions need to be captured together, ideally in a cycle-latency-accurate performance simulator, so different cache hierarchy configurations can be evaluated against actual performance targets. I built [[https://thecloudlet.github.io/technical/project/stratum/][Stratum]] for exactly this — a configurable cache simulator where hierarchy topology is bound at compile time, mirroring how memory systems are fixed at hardware synthesis.
...
This is *[[https://en.wikipedia.org/wiki/Layout_Versus_Schematic][LVS (Layout vs Schematic)]]*. An [[https://en.wikipedia.org/wiki/Electronic_design_automation][EDA (Electronic Design Automation)]] tool like Calibre reads the GDS polygons, infers transistors and connections, and compares the resulting netlist against the schematic netlist you generated. One mismatched net name, one missing transistor → LVS fails → tapeout blocked.
...
Increasing EMA gives the bitcell more time to pull down the bitline → larger differential voltage → better Read [[https://en.wikipedia.org/wiki/Noise_margin][SNM (Static Noise Margin)]] → better yield, at the cost of slightly longer access time. This is a three-way trade-off between area, speed, and yield.
...
ML compiler tiling (e.g., [[https://en.wikipedia.org/wiki/General_matrix_multiply][GEMM (General Matrix Multiplication)]] on an [[https://en.wikipedia.org/wiki/AI_accelerator][NPU (Neural Processing Unit)]]) takes a large computation and cuts it into pieces that fit the hardware:
