#+TITLE: From Pixels to Tensors, Part 4: Connectionism
...
[[file:part-3-symoblic-ai.org][Part 3]] ended on an exit. Symbolic AI hit the limit of what a human can foresee
...
<figcaption style="font-size:0.9rem;opacity:0.75;margin-top:0.6rem">The four circuits of Figure 1(a)–(d). Solid arrowhead: excitatory synapse. Open circle: inhibitory synapse. θ is the neuron's threshold.</figcaption>
</figure>
#+END_EXPORT

Circuits (b) and (c) are wired identically: two excitatory synapses arriving at
one neuron. The difference is the threshold. At θ = 1 the neuron fires when
either input fired; at θ = 2 only when both did. Disjunction and conjunction are
one mechanism — count the active excitatory synapses, compare against θ — at two
parameter settings.
...
are no circles, the process cannot return to a neuron it already expanded, so it
runs out of substitutions in a finite number of steps, and what is left mentions
only the inputs.
...
<line x1="89" y1="125" x2="306" y2="158" stroke="currentColor" stroke-width="1.5" marker-end="url(#t1-exc)"/>
<line x1="89" y1="180" x2="304" y2="172" stroke="currentColor" stroke-width="1.5" marker-end="url(#t1-inh)"/>
...
such a sentence is a TPE, and therefore realizable, *unless* it is false when
all its constituents are assumed false. Equivalently: unless its Hilbert
disjunctive normal form contains a term made exclusively of negated terms.[fn:1]

#+BEGIN_EXPORT html
<figure style="margin:2rem 0">
<svg viewBox="0 0 720 250" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Theorem 3: all sentences over firing events, nearly all realizable, contradictions excluded" style="width:100%;height:auto;color:inherit">
...
<rect x="66" y="70" width="470" height="124" rx="10" fill="none" stroke="currentColor" stroke-width="2"/>
<text x="301" y="118" text-anchor="middle" font-size="13" fill="currentColor" font-weight="600">realizable by a net of order 0</text>
<text x="301" y="142" text-anchor="middle" font-size="10.5" fill="currentColor" opacity="0.75">is a TPE — Theorem 2 builds the net</text>
<text x="301" y="164" text-anchor="middle" font-size="10.5" fill="currentColor" opacity="0.75">“nearly everything you can say”</text>
<rect x="556" y="70" width="104" height="124" rx="10" fill="none" stroke="currentColor" stroke-width="1.6" stroke-dasharray="4 3"/>
<text x="608" y="112" text-anchor="middle" font-size="11" fill="currentColor">contra-</text>
<text x="608" y="128" text-anchor="middle" font-size="11" fill="currentColor">dictions</text>
<text x="608" y="152" text-anchor="middle" font-size="9" fill="currentColor" opacity="0.7">p ∧ ¬p</text>
<text x="608" y="168" text-anchor="middle" font-size="9" fill="currentColor" opacity="0.7">false always</text>
<text x="360" y="236" text-anchor="middle" font-size="10" fill="currentColor" opacity="0.75">The only sentences no net can realize are the ones that cannot be true of anything.</text>
</svg>
<figcaption style="font-size:0.9rem;opacity:0.75;margin-top:0.6rem">Theorem 3 widens the result from the restricted TPE grammar to ordinary logical writing. The excluded region is exactly the logically false sentences.</figcaption>
</figure>
#+END_EXPORT

The excluded case is the correct answer. A sentence like \(p(z_1) . \sim
p(z_1)\) asks for a neuron that fires and does not fire at the same instant, and
no physical arrangement realizes it. The equivalence between nets and logic
covers everything statable about who fired when, minus the statements that
...
McCulloch and Pitts do come back to changing synapses, later, in the section on
nets with circles. Give them a net whose synapses change and they hand back an
equivalent net with fixed structure and a loop in it. Learning gets absorbed
into something assumption (5) already allows. That tells you what a fixed
structure can imitate. It gives a net no way to change itself, and that gap is
what the next fifteen years are about.
...
universal rule. That is the move the authors flagged when they said the
difference was immaterial to their argument.
...
defining the error. Fifteen years separate the two papers. The mechanism arrives
with Rosenblatt's perceptron in 1958.
...
[fn:1] McCulloch, W.S. & Pitts, W. (1943). [[https://www.csulb.edu/~cwallis/382/readings/482/mccolloch.logical.calculus.ideas.1943.pdf]["A Logical Calculus of the Ideas
Immanent in Nervous Activity"]]. /Bulletin of Mathematical Biophysics/ 5, 115–133.
Reprinted in /Bulletin of Mathematical Biology/ 52 (1990), 99–115; the reprint
pagination is the one I read from. The five assumptions are on p. 101, Theorem 1
on p. 102, the TPE recursion and general formula on p. 103, Theorems 2 and 3
on p. 104, and the Figure 1 circuits on p. 105. Section 3, on nets *with*
circles, is where recursive functions and memory enter; the authors call that
case "very much more difficult" (p. 108) and it is not covered here.
...
(1961), published in /General Semantics Bulletin/ Nos. 26–27. Collected in
